...
then
	...
	if [ $count -eq 0 ] ; 
		...
		mkdir -p ./Report/$artifacType ./Errors/$artifacType
        if [ $(echo $?) -eq 1 ]
		then
		   exit 1
		fi
		...
		do
			...
			if [ $(echo $?) -eq 2 ] # Si no puede abrir el .log entonces arroja error
			 ...
			 then
                ...
                if grep -w -i 'ERROR' $log_report  
 
                then
					
					grep -w -i 'ERROR' $log_report > $log_error
					
					rm -rf $sourceFolder/Backup/$artifacType/$listArtifacType ## Recuerde Borrar
            ...
            if [ $(echo $?) -eq 1 ] # Si no puede abrir el archivo .error entonces fue OK
             
			 then
                echo '\n >>> '$artifacType'/'$fileXML' [IMPORT OK]'
            else 

                echo '\n >>> '$artifacType'/'$fileXML' [IMPORT FAILED]'
                exit 1
